...
public class forma_pago extends JDialog {
	...
	public static void main(String[] args) {
		try {
			forma_pago dialog = new forma_pago(null);
	 ...
	 * Create the dialog.
	 */
	public forma_pago(String cod) {
		fp= this;
		this.placa = cod;
		System.out.println("Placa formapago: "+placa);
		...
		contentPanel.setLayout(null);
		{
			...
			pn_autos.add(btn_add);
			{
				...
				btn_save.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
							...
							int treshold = 2022 - Integer.parseInt(txt_anio.getText());
							double valor_origital = Double.parseDouble(txt_precio.getText());
							double pre_aux = Double.parseDouble(txt_precio.getText());
						
							pre_aux = pre_aux - (((valor_origital*1.5)/100)*treshold); //resta por año
							if(Integer.parseInt(txt_klm.getText()) > 80000){
								pre_aux = pre_aux - ((valor_origital*2.5)/100); // resta del kilometraje
							}
							if(Integer.parseInt(spn_nDuenio.getValue()+"")>1) { //resta por numeros de dueño
								pre_aux = pre_aux - (((valor_origital*2)/100)*Integer.parseInt(spn_nDuenio.getValue()+""));
							}
							int choque=-1;
							int aire=-1;
							if(rbtn_si1.isSelected()) {
								choque = 1; //tiene choque
								pre_aux = pre_aux - ((valor_origital*15)/100);
							...
							if(rbtnsi2.isSelected()) {
								...
								pre_aux = pre_aux - ((valor_origital*0.5)/100);			
							}
							
							if(cmb_tapiceria.getSelectedIndex() ==0) {
								pre_aux = pre_aux + ((valor_origital*1)/100);	
							}
							else if(cmb_tapiceria.getSelectedIndex() ==1){
								pre_aux = pre_aux - ((valor_origital*1.5)/100);	
							}
							else if(cmb_tapiceria.getSelectedIndex() ==2) {
								pre_aux = pre_aux - ((valor_origital*3)/100);	
							}
							

							if(cmb_cajaCambios.getSelectedIndex() ==0) { //Pintura
								pre_aux = pre_aux + ((valor_origital*2)/100);	
							}
							else if(cmb_cajaCambios.getSelectedIndex()==1){
								pre_aux = pre_aux - ((valor_origital*2)/100);	
							}
							else if(cmb_cajaCambios.getSelectedIndex() ==2) {
								pre_aux = pre_aux - ((valor_origital*10)/100);	
							}
							
							if(cmb_carroceria.getSelectedIndex() ==0) {
								pre_aux = pre_aux + ((valor_origital*2)/100);	
							}
							else if(cmb_carroceria.getSelectedIndex()==1){
								pre_aux = pre_aux - ((valor_origital*2)/100);	
							}
							else if(cmb_carroceria.getSelectedIndex() ==2) {
								pre_aux = pre_aux - ((valor_origital*10)/100);	
							}
							
							
							if(treshold<=10) {
								System.out.println(pre_aux);
								//JOptionPane.showMessageDialog(null, "Vehiculo dentro del rango");
								...
								if(rdbtn_nuevo.isSelected() && choque !=-1 && aire !=-1 && img1 && img2 && img3) {
									...
									JOptionPane.showMessageDialog(null, "El precio final de su vehiculo sera: "+pre_aux);
									comprar_auto ca = new comprar_auto(placa,txt_placa.getText(),pre_aux);
									ca.setVisible(true);
									fp.setVisible(false);
								}
								...
								JOptionPane.showMessageDialog(null, "El año del vehículo supera a los 10 años limites");
							}
			...
			btnCancelar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					comprar_auto ca = new comprar_auto(placa,"",0.0);
					ca.setVisible(true);
					fp.setVisible(false);
				}
			});
			btnCancelar.setBounds(158, 710, 117, 25);
			pn_autos.add(btnCancelar);
		}
		{
